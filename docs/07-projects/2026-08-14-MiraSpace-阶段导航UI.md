...
> **不改变**模拟规则；**不**新增科学阶段。  
> **配套**：[性能与时间控制 UI](2026-08-14-MiraSpace-性能与时间控制UI.md)（时间倍率、右下角操作区）— 可与本任务 **同一 PR** 交付。  
...
```
┌─────────────────────────────────────────────────────────┐
│ [阶段导航]  原始汤 | 复制子 | 原细胞 | 化学子 | 多细胞  │  ← 固定顶栏
├─────────────────────────────────────────────────────────┤
│  HUD 指标（只读）                                        │
│                    Canvas 全屏                           │
│                              ┌ 暂停·倍率·视图·重置 ┐   │  ← 右下角（见性能与时间控制 UI）
└─────────────────────────────────────────────────────────┘
```

- **横屏优先**；竖屏时导航可折行，不挡 canvas 主区域。
- **操作按钮**（暂停、倍率、网格、场、重置）**不**放在顶栏或 HUD，见 [性能与时间控制 UI](2026-08-14-MiraSpace-性能与时间控制UI.md)。
...
在 MiraSpace 实现单页阶段导航（N0→N1）+ 时间控制（T0→T1，推荐同 PR）：
- stage-nav.js：5 Tab 映射 stage0/2/3/4/5-default，保留 seed，URL sync
- sim-clock.js + control-panel.js：1×/5×/20×、暂停、右下角网格/场/重置
- 切换时 reset 世界，HUD 指标组按阶段显隐；操作按钮迁出 HUD
